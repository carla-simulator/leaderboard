...
from ros_compatibility import get_service_request, ROS_VERSION, ServiceException
...
class AgentWrapper(object):
    ...
    def __init__(self, agent):
        ...
        """
        self._agent = agent 
        self.spawn_object_service = self._agent.ros_node.create_service_client("/carla/spawn_object", SpawnObject)
        self.destroy_object_service = self._agent.ros_node.create_service_client("/carla/destroy_object", DestroyObject)
    ...
    def setup_sensors(self, vehicle, debug_mode=False):
        ...
        :return:
        """
        self._agent.ros_node.logwarn(f"Setting up sensors...")
        ...
        for sensor_spec in self._agent.sensors():
            ...
            attributes = []
            # These are the pseudosensors, created throught ros_bridge service
            ...
            if spawn_object_type is not None:
                ...
                response = self._agent.ros_node.call_service(self.spawn_object_service, spawn_object_request, timeout_ros2=5.0, executor=self._agent.leaderboard_executor)
                self._sensors_list.append(response.id)
            else:
    ...
    def cleanup(self):
        ...
        for i, _ in enumerate(self._sensors_list):
            if self._sensors_list[i] is not None:
                if isinstance(self._sensors_list[i], int):
                    try:
                        destroy_object_request = get_service_request(DestroyObject)
                        destroy_object_request.id = self._sensors_list[i]
                        self._agent.ros_node.call_service(self.destroy_object_service,
                                destroy_object_request, timeout_ros2=1.0)
                    except ServiceException as e:
                        self._agent.ros_node.logwarn(f'Could not call destroy service on objects, error: {e}')
                else:
